    ...
    try {
        db_1.con.query("SELECT ID, image FROM slider_image WHERE id=" + id, function (error, results, fields) {
            if (error)
                ...
                fs_1.default.unlink(results[0].image.replace('uploads', 'uploads/'), function (error) {
                    if (error)
                        throw error;
                });
                db_1.con.query(query, function (error, results, fields) {
...
exports.default = { addSliderImage: addSliderImage, deleteSliderImage: deleteSliderImage, updateSliderImage: updateSliderImage, getAllSliderImages: getAllSliderImages };
